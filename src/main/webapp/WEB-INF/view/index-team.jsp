<div class="section" id="team">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <h1 class="text-left"><fmt:message key="index.team.title"/></h1>
                <p class="lead text-left"><fmt:message key="index.team.subtitle"/></p>
            </div>
        </div>
        <div class="col-md-6 animate" data-animation="slideRight">
            <div class="row team-row">
                <div class="col-sm-4">
                    <img class="img-circle img-responsive" src="<c:url value="/static/images/team/gm.jpeg"/>">
                </div>
                <div class="col-sm-8">
                    <h3 class="text-left font-bold"><fmt:message key="index.team.developer.1"/></h3>
                    <p class="lead text-left"><fmt:message key="index.team.developer"/></p>
                </div>
            </div>
            <div class="row team-row">
                <div class="col-sm-4">
                    <img src="<c:url value="/static/images/team/gm.jpeg"/>" class="img-circle img-responsive">
                </div>
                <div class="col-sm-8">
                    <h3 class="text-left"><fmt:message key="index.team.developer.1"/></h3>
                    <p class="lead text-left"><fmt:message key="index.team.developer"/></p>
                </div>
            </div>
            <div class="row team-row">
                <div class="col-sm-4">
                    <img src="<c:url value="/static/images/team/gm.jpeg"/>" class="img-circle img-responsive">
                </div>
                <div class="col-sm-8">
                    <h3 class="text-left"><fmt:message key="index.team.developer.1"/></h3>
                    <p class="lead text-left"><fmt:message key="index.team.developer"/></p>
                </div>
            </div>
        </div>
        <div class="col-md-6 animate" data-animation="slideLeft">
            <div class="row team-row">
                <div class="col-sm-4">
                    <img src="<c:url value="/static/images/team/gm.jpeg"/>" class="img-circle img-responsive">
                </div>
                <div class="col-sm-8 text-center">
                    <h3 class="text-left"><fmt:message key="index.team.developer.1"/></h3>
                    <p class="lead text-left"><fmt:message key="index.team.developer"/></p>
                </div>
            </div>
            <div class="row team-row">
                <div class="col-sm-4">
                    <img src="<c:url value="/static/images/team/gm.jpeg"/>" class="img-circle img-responsive">
                </div>
                <div class="col-sm-8">
                    <h3 class="text-left"><fmt:message key="index.team.developer.1"/></h3>
                    <p class="lead text-left"><fmt:message key="index.team.developer"/></p>
                </div>
            </div>
            <div class="row team-row">
                <div class="col-sm-4">
                    <img src="<c:url value="/static/images/team/gm.jpeg"/>" class="img-circle img-responsive">
                </div>
                <div class="col-sm-8 text-center">
                    <h3 class="text-left"><fmt:message key="index.team.developer.1"/></h3>
                    <p class="lead text-left"><fmt:message key="index.team.developer"/></p>
                </div>
            </div>
        </div>
    </div>
</div>